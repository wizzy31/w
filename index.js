...
const vcard = 'BEGIN:VCARD\n'
            ...
            + 'FN:Wizzy\n' // Nama kamu
            + 'ORG:Lexa;\n' // Nama bot
            + 'TEL;type=CELL;type=VOICE;waid=558681140024:+55 868114-0024\n' //Nomor whatsapp kamu
            + 'END:VCARD'
prefix = ','
blocked = []
limitawal = '999'
	...
	client.on('chat-update', async (mek) => {
		try {
			...
			mess = {
				...
				Iv: 'Desculpe link invalido☹️'
				},
				only: {
					group: '[❗] Este comando só pode ser usado em grupos! ❌',
					premium: '[❗] ESTE PEDIDO É SO PARA *USUÁRIOS PREMIUM*',
					mod: '[❗] ESTE PEDIDO É ESPECÍFICO PARA O MODERADOR*',
					benned: 'Você está bloqueado, Contate o proprietário para rever seu desbloqueio',
					...
					userB: `──「 LISTA 」──\nOlá !\nDesculpe, Você não está registrado no banco de dados. Registre-se para fazer amizade comigo, \n\nCommand : ${prefix}daftar nome|idade\nExemplo : ${prefix}daftar Lucas|14\n\n──「 FIG ZAP BOT 」──`,
			...
			const premium = ["558681140024@s.whatsapp.net","559784130980@s.whatsapp.net"]
            ...
            case 'rapL':
				...
				case 'daftar':
					...
					var umure = reg.split("|")[1];
						...
						client.sendMessage(from, `\`\`\`O registro foi bem sucedido! SN: TM08GK8HEUZBEHDO\`\`\`\n\n\`\`\`data ${date} ${time}\`\`\`\n\`\`\`[Nome]: ${jeneng}\`\`\`\n\`\`\`[Número]: wa.me/${sender.split("@")[0]}\`\`\`\n\`\`\`[Idade]: ${umure}\`\`\`\n\`\`\`Para usar o bot\`\`\`\n\`\`\`Por favor\`\`\`\n\`\`\`enviar ${prefix}help\`\`\`\n\`\`\`\nTotal de usuários ${user.length}\`\`\``, text, {quoted: mek})
					break
				case 'hunti':
					reply(mess.wait)
...
// recursos anime, random @nabuto domina vida
          ...
          reply('Espere..')
			    ...
			    case 'dono':
                    ...
                    client.sendMessage(from, 'Meu dono é esse gasoso aí tal de "Wizzy"🙄, chama ele aí ele diz que é "Editor"',MessageType.text, { quoted: mek} )
                    break
                case 'fitnah':	
				case 'fake':          
                    if (!isGroup) return reply(mess.only.group)
				...
				case 'dono':
					...
					client.sendMessage(from, buffer, image, {quoted: mek, caption: '*CRIADOR:* WIZZY\n*INSTAGRAM:* https://www.instagram.com/_wi22y/ \n*WPP:* wa.me/+558681140024\n*\n\nEspero que gostem do bot️'})
					break
				case 'setprefix':
					if (args.length < 1) return
					if (!isOwner) return reply(mess.only.ownerB)
